...
在设置页面，如图 7-7 所示，我们选择的项目类型为 Web application，AIR 应用的基本知识，可以参考developerWorks 的另一篇文章，《使用 Adobe AIR 和 Dojo 开发基于 Ajax 的 Mashup 应用》。Application server type 我们以J2EE为例，不使用 Use remote object access servic。这些先进内容不在这篇文章的讨论范围。最后，我们使用 Eclipse Ganymede J2EE 版本内置的 WTP(Web Tools Platform) 来创建一个后端使用 Java 前端使用 Flex 的 RIA 项目。在默认设置下，src 是 Java 代码的源代码文件夹。
...
如果我们想在运行时观察变量的值，就像调试java代码一样要怎么做呢？首先，我们添加断点在我们的 ActionScript 代码就像我们在java代码中设置断点。如图 7-14 所示：
